      ...
      if (status !== "success") {
        ...
        window.setTimeout((function() {
          return mainPage.evaluate(function() {
            ...
            jQuery("input,label,span").each(function(index, item) {
              var fontSize;
              fontSize = parseInt(jQuery(item).css("font-size"));
              fontSize = (fontSize - 1) + "px";
              return jQuery(item).css("font-size", fontSize);
            });
            return jQuery('textarea').each(function(index, item) {
              var div_with_textarea_content;
              if (jQuery(item).val().length > 0) {
                div_with_textarea_content = jQuery('<div />').css('border', '1px solid black').css('margin', '3px').css('padding', '3px').html(jQuery(item).val().replace(/(\r\n|\n\r|\r|\n)/g, "<br>"));
                return jQuery(item).replaceWith(div_with_textarea_content);
              }
            });
          });
        }), 5000);
